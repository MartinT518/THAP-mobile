...
function getMapsConfig(): MapsConfig {
  const baseUrl = ENV.servicesBaseUrl;
  const apiKey = ENV.servicesApiKey;

  if (!baseUrl || !apiKey) {
    throw new Error(
      "Google Maps proxy credentials missing: set THAP_SERVICES_BASE_URL and THAP_SERVICES_API_KEY"
    );
  }
